...
describe("Token Vesting", function() {
  ...
  it("Should test a Multi Lock Type 2", async function() {
    ...
    var lockParams = [
      [this.alice.address, '500', block.timestamp + 12, fiveMinsFromNow, ethers.ZeroAddress],
      [this.bob.address, '250', block.timestamp + 100, fiveMinsFromNow, ethers.ZeroAddress],
      [this.bob.address, '125', block.timestamp + 36, fiveMinsFromNow, ethers.ZeroAddress],
      [this.alice.address, '100', block.timestamp, fiveMinsFromNow, ethers.ZeroAddress],
    ]
  ...
  it("Should test Multi Lock withdrawls Type 2 withdrawls", async function() {
    ...
    await this.TOKEN_VESTING.connect(this.alice).withdraw(0, '60')
    await expect(this.TOKEN_VESTING.connect(this.alice).withdraw(0, '440')).to.be.revertedWith("AMOUNT")
    await hre.ethers.provider.send("evm_increaseTime", [lock1.endEmission - block.timestamp])
    await hre.ethers.provider.send("evm_mine")

    await this.TOKEN_VESTING.connect(this.alice).withdraw(0, '440')
    ...
    await this.TOKEN_VESTING.connect(this.alice).withdraw(3, '100')
    expect(await this.token.balanceOf(this.alice.address)).to.equal(alicePreBalance + 600);
  })
});
